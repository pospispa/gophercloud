...
// deployment.
func V2EndpointURL(catalog *tokens2.ServiceCatalog, opts gophercloud.EndpointOpts) (gophercloud.EndpointInfo, error) {
	// Extract Endpoints from the catalog entries that match the requested Type, Name if provided, and Region if provided.
	var endpoints = make([]tokens2.Endpoint, 0, 1)
	var foundEndpoint gophercloud.EndpointInfo
	...
	for _, endpoint := range endpoints {
		...
		case gophercloud.AvailabilityPublic:
			foundEndpoint.Initialize(endpoint, gophercloud.V20)
...
// need to specify a Name and/or a Region depending on what's available on your OpenStack
// deployment.
func V3EndpointURL(catalog *tokens3.ServiceCatalog, opts gophercloud.EndpointOpts) (gophercloud.EndpointInfo, error) {
	...
	// Report an error if the options were ambiguous.
	if len(endpoints) > 1 {
		return nil, ErrMultipleMatchingEndpointsV3{Endpoints: endpoints}
	}

	// Extract the URL from the matching Endpoint.
	for _, endpoint := range endpoints {
		foundEndpoint.Initialize(endpoint, gophercloud.V30)
		return foundEndpoint, nil
	}
